...
export default function Chat() {
  ...
  // Restore session from localStorage if no URL session
  useEffect(() => {
    if (!urlSessionId) {
      const savedSession = localStorage.getItem('voya_session_id');
      if (savedSession) {
        navigate(`/plan/${savedSession}`, { replace: true });
      }
    }
      ...
      fetch(`${API}/session/${urlSessionId}`)
        ...
        .then(data => {
          if (data.success && data.session) {
            ...
            const hist = data.session.history || [];
            setMessages(
              hist.map(h => ({ role: h.role, content: h.content }))
            );
            if (data.itinerary) {
              setItinerary(data.itinerary.data || data.itinerary);
              setShowItinerary(true);
            }
            localStorage.setItem('voya_session_id', urlSessionId);
          }
        })
        .catch(() => {});
    }
  ...
  // Send initial message from landing page (StrictMode safe)
  useEffect(() => {
    const init = location.state?.initialMessage;
    if (init && messages.length === 0 && !initSentRef.current) {
  ...
  const sendMessage = useCallback(async (text, imageData = null) => {
    ...
    try {
      ...
      if (data.session_id) {
        ...
        localStorage.setItem('voya_session_id', data.session_id);
        if (!sessionId) {
